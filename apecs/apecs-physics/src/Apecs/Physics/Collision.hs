...
addPostStepCallback :: (Has w m Physics, MonadIO m) => Int -> SystemT w IO () -> SystemT w m Bool
    ...
    withForeignPtr spcPtr $ \space -> do
      alloca $ \keyPtr -> do
        ...
        success <-
          [C.block| int {
            int *data = 0;
            return cpSpaceAddPostStepCallback($(cpSpace *space), $(void*fn),$(void*keyPtr), &data);
          } |]
        pure (success > 0)
