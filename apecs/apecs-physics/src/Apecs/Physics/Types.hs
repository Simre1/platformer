...
-- Force should be used for an effect that spans over a period of time, while Impulse be used for instantenous effects.
newtype Force = Force Vec

-- | The impulse is applied to the body's center of gravity and reset to @ Vec 0 0 @ after every simulation step.
-- Impulse represents a quick/instantenous exertion of force (player jumping/colliding with a wall).
newtype Impulse = Impulse Vec
...
newtype ConstraintImpulse = ConstraintImpulse Double deriving (Eq, Show)
